...
;;; Fix Me: What's the difference between parent and ensemble?
...
;;;
;;; Ensembles: collections of components
;;; Components: have input and output ports as well pre-, post- and invariant conditions
;;;             have an ensemble-type which is either:
;;;                  :primitive, meaning it's not decomposable
;;;                  the name of an ensemble which can be instantiated if desired to 
;;;                      model the internal workings of that component
;;;                      In this case there is also an instantiation mapping connecting
;;;                         the inputs of the component to the inputs of its initial components
;;;                         and the outputs of its terminal parts to the outputs of the component
;;;                   A Behavior model can be provided for the type name also, 
...
;;;
;;;   When an ensemble is instantiated to represent the innards of a component there is an 
;;;     assertion made to represent the refinement and dataflow and control flow
;;;     assertions to connect up everything.
;;;
...
(define-predicate component (ensemble component-name component) (ltms:ltms-predicate-model))
...
(define-predicate corresponding-ensemble (component ensemble) (ltms:ltms-predicate-model))
...
(define-predicate attack (ensemble attack-name attack-type) (ltms:ltms-predicate-model))
...
(defrule induce-attack (:forward)
  If [and [resource ?ensemble ?resource-name ?resource]
          ...
          [attack ?ensemble ?name ?attack-type]]
...
(defclass print-type-and-name-mixin ()
  ((object-name :accessor object-name :initarg :name)
...
(defmethod print-object ((thing print-type-and-name-mixin) stream)
  (format stream "<~s ~a>" (type-of thing) (object-name thing)))

(defclass ensemble (print-type-and-name-mixin)
  ((components :accessor components :initarg :components :initform nil)
   ))

(defclass situation-object (print-type-and-name-mixin)
...
(defclass interval-object (print-type-and-name-mixin)
  ((component :accessor component :initarg :component)
   (assertions :accessor assertions :initarg :assertions :initform nil)))

(defmethod before-or-after ((i interval-object)) 'during)

(defclass part-of-ensemble-mixin ()
  ((ensemble :accessor ensemble :initarg :ensemble)))

(defmethod initialize-instance :after ((c part-of-ensemble-mixin) &rest initargs)
  (let* ((name (object-name c))
	 (ensemble (ensemble c)))
    (tell `[component ,ensemble ,name ,c])
    (when ensemble
      (push c (components ensemble)))))
...
(defclass primitive-branch-mixin (has-parent-mixin 
				  part-of-ensemble-mixin
				  print-type-and-name-mixin)
...
(defclass component-object (part-of-ensemble-mixin
			    has-parent-mixin 
			    has-presituation-mixin 
			    has-postsituation-mixin
			    has-interval-mixin
...
(defmethod qualified-name :around ((c component-object))
  (let ((answer (call-next-method)))
    (unless answer
      (let ((ensemble (ensemble c)))
	(cond ((not (null ensemble))
	       (setq answer (make-name (qualified-name (corresponding-component ensemble)) (object-name c) ".")))
	      (t (setq answer (object-name c)))))
      (setf (qualified-name c) answer))
    answer))

(defmethod initialize-instance :after ((c component-object) &key )
  (tell `[component-type ,c component]))

;;; The idea is that this ensemble is the "insides" of the corresponing-component
;;; The corresponding component points to the parent ensemble
(defclass nested-ensemble (ensemble)
  ((corresponding-component :accessor corresponding-component :initarg :corresponding-component)))

(defmethod ensemble ((sub-ensemble nested-ensemble))
  (ensemble (corresponding-component sub-ensemble)))

(defclass resource-object (print-type-and-name-mixin)
  ((name :accessor resource-object :initarg :name)
...
(defmethod qualified-name :around ((r resource-object))
  (let ((answer (call-next-method)))
    (unless answer
      (let ((ensemble (ensemble r)))
	(cond ((not (null ensemble))
	       (setq answer (make-name (qualified-name ensemble) (object-name r) ".")))
...
(defmethod handle-null-cases ((component component-object))
  (ask `[corresponding-component ,component ?ensemble]
       #'(lambda (just)
	   (declare (ignore just))
	   (handle-null-cases ?ensemble))))

(defmethod handle-null-cases ((ensemble ensemble))
  (ask `[component ,ensemble ?name ?component]
       #'(lambda (just)
	   (declare (ignore just))
	   (when (has-no-incoming-controlflows ?component)
		   ...
		   (ask [situation ?other-component ?time ?other-situation]
			#'(lambda (just2)
			    (when (or (eql ?time 'before)
				      (and (eql ?time 'after)
					   (eql (corresponding-component (ensemble component)) ?other-component)))
			      (let* ((statement (predication-statement self))
...
(defmethod build-inside :after (type component)
  (declare (ignore type))
  (push-ensemble-initial-assertions component))

(defun push-ensemble-initial-assertions (outside)
  (let* ((before-situation (block block
			     (ask `[situation ,outside before ?before-situation]
				  #'(lambda (just1)
				      (declare (ignore just1))
				      (return-from block ?before-situation)))))
	 (ensemble (block block
		     (ask `[corresponding-ensemble ,outside ?ensemble]
			  #'(lambda (just)
			      (declare (ignore just))
			      (return-from block ?ensemble)))))
	 (assertions (assertions before-situation)))
    ;; for each input of the box
    (loop for assertion in assertions
	do (push-outside-assertion-inside assertion ensemble))))
      
(defun push-outside-assertion-inside (assertion ensemble)
  (flet ((push-assertion (assertion new-situation support)
	...
	do (ask `[input ?input ,ensemble ,value]
	       #'(lambda (just2)
		   (declare (ignore just2))
		   ;; and data flow from that input
		   (ask `[dataflow ?input ,ensemble ?other-input ?component]
			#'(lambda (just3)
			    ;; to a component of the outer box
			    (ask `[and [corresponding-ensemble ,ensemble ?inside]
				       [component ?inside ? ?component]]
				 #'(lambda (just4)
				     (declare (ignore just4))
				     (ask [situation ?component before ?other-sit]
					  #'(lambda(just5)
...
(define-predicate-model has-bit-position-mixin
	...
	()
  #+genera (:writable-instance-variables (bit-pattern bit-pattern)))
...
(defvar *ensembles* nil)

(defgeneric build-outside (module-type role-name &optional parent))

(defmethod build-outside :around (module-type role-name &optional the-parent)
...
(defgeneric build-inside (module-type corresponding-component))
...
(defrule resource-hierarchical (:backward)
  then [resource ?ensemble ?resource-name ?resource]
  if [and [component ?ensemble ? ?component]
...
(defmacro define-ensemble (type 
  ...
  `(eval-when (:compile-toplevel :execute :load-toplevel)
     (when ,top-level (pushnew ',type *ensembles*))
     ...
     (defmethod build-outside ((type (eql ',type)) name &optional parent)
       (let ((component (make-instance 'component-object
	 ...
	 component))
     (defmethod build-inside ((type (eql ',type)) corresponding-component)
       (let* ((ensemble (make-instance 'nested-ensemble 
			  :name (object-name corresponding-component)
			  :type ',type)))
	 ;; symmetric pointers
	 (setf (inside corresponding-component) ensemble
	       (corresponding-component ensemble) corresponding-component)
	 (tell `[corresponding-ensemble ,corresponding-component ,ensemble])
	 ...
	 ,@(loop for (role-name . plist) in components
	       ...
	       collect `(let ((component (build-outside ',type ',role-name ensemble)))
			  (build-exhaustives component ',models)
			  ,@(loop for model in models 
				collect `(tell `[possible-model ,component ,',model]))))
	 ...
	 ,@(loop for (from-port-name from-component-name to-port-name to-component-name) in controlflows
		...
		((eql to-component-name type)
		 `(make-controlflow ensemble ',from-component-name ',from-port-name corresponding-component ',to-port-name))
		((eql from-component-name type)
		 `(make-controlflow ensemble corresponding-component ',from-port-name ',to-component-name ',to-port-name))
		(t
		 `(make-controlflow ensemble ',from-component-name ',from-port-name ',to-component-name ',to-port-name))))
	 ,@(loop for (from-port-name from-component-name to-port-name to-component-name) in dataflows
	       collect (cond 
			((eql to-component-name type)
			 `(make-dataflow ensemble ',from-component-name ',from-port-name corresponding-component ',to-port-name))
			((eql from-component-name type)
			 `(make-dataflow ensemble corresponding-component ',from-port-name ',to-component-name ',to-port-name))
			(t  `(make-dataflow ensemble ',from-component-name ',from-port-name ',to-component-name ',to-port-name))))
       (handle-null-cases ensemble)
       ensemble))))

(defun make-controlflow (ensemble from-component-name from-port-name to-component-name to-port-name)
  (cond 
   ((typep to-component-name 'component-object)
    (ask `[component ,ensemble ,from-component-name ?from-component]
	 #'(lambda (just)
	     (declare (ignore just))
	     (tell `[controlfow ,from-port-name ?from-component ,to-port-name ,to-component-name]))))
   ((typep from-component-name 'component-object)
    (ask `[component ,ensemble ,to-component-name ?to-component]
	 #'(lambda (just)
	     (declare (ignore just))
	     (tell `[controlflow ,from-port-name ,from-component-name ,to-port-name ?to-component]))))
   (t (ask `[and [component ,ensemble ,from-component-name ?from-component]
		 [component ,ensemble ,to-component-name ?to-component]]
	   #'(lambda (just)
	       (declare (ignore just))
	       (tell `[controlflow ,from-port-name ?from-component ,to-port-name ?to-component]))))))

(defun make-dataflow (ensemble from-component-name from-port-name to-component-name to-port-name)
  (cond 
   ((typep to-component-name 'component-object)
    (ask `[component ,ensemble ,from-component-name ?from-component]
	 #'(lambda (just)
	     (declare (ignore just))
	     (tell `[dataflow ,from-port-name ?from-component ,to-port-name ,to-component-name]))))
   ((typep from-component-name 'component-object)
    (ask `[component ,ensemble ,to-component-name ?to-component]
	 #'(lambda (just)
	     (declare (ignore just))
	     (tell `[dataflow ,from-port-name ,from-component-name ,to-port-name ?to-component]))))
   (t (ask `[and [component ,ensemble ,from-component-name ?from-component]
		 [component ,ensemble ,to-component-name ?to-component]]
	   #'(lambda (just)
	       (declare (ignore just))
...
(defmacro define-resource-type (type-name &key modes vulnerabilities prior-probabilities)
  `(defmethod Instantiate-resource ((type (eql ',type-name)) resource-name owning-component)
     (let ((resource (make-instance 'resource-object :type type :name resource-name :ensemble owning-component :owner owning-component :users (list owning-component))))
    ...
    `(eval-when (:compile-toplevel :execute :load-toplevel)
       ,@(loop for (branch-name condition) in pairs
	     ...
	     collect `(defrule ,rule-name (:Forward)
			if [and [component ?ensemble ?name ?split]
...
(defrule in-in-dataflow (:forward)
  If [and [dataflow ?producer-port-name ?producer ?consumer-port-name ?consumer]
	  ...
	  [corresponding-ensemble ?producer ?ensemble]
	  [component ?ensemble ?name ?consumer]
...
(defrule out-out-dataflow (:forward)
  If [and [dataflow ?producer-port-name ?producer ?consumer-port-name ?consumer]
	  ;; this only happens between component and parent
	  [component ?ensemble ?name ?producer]
	  [corresponding-ensemble ?consumer ?ensemble]
          [port output ?producer ?producer-port-name]
...
(defmethod parent-component ((c component-object))
  (let ((e (ensemble c)))
    (when e (corresponding-component e))))
...
(defun most-specific-component (c-set)
    (labels ((kill-his-parents (c)
	       (let ((his-parent (parent-component c)))
		 (when his-parent 
  ...
  `(progn
     ...
     (defun ,name (ensemble)
       ,@(when (and (null attack-types) (null parents))
           `((declare (ignore ensemble))))
       ,@(loop for (attack-type a-priori-probability) in attack-types
               collect `(tell `[attack ,ensemble ,',name ,',attack-type] :justification :premise)
       ...
       ,@(loop for parent in parents
               collect `(apply-attack-model ',parent ensemble))
       )))

(defun apply-attack-model (name ensemble)
  (funcall name ensemble)) 
...
(defun build-nodes (ensemble ideal-diagram node-type)
  (ask `[,node-type ,ensemble ?name ?node]
       #'(lambda (stuff)
	   (declare (ignore stuff))
	   (let ((states nil)
...
(defun do-priors (ensemble ideal-diagram)
  (ask `[and [resource ,ensemble ? ?resource]
...
(defun do-conditional-probabilities (ensemble ideal-diagram)
  ;; first add arcs
  (ask `[component ,ensemble ? ?component]
       #'(lambda (stuff)
	   (declare (ignore stuff))
...
(defun do-attack-conditional-probabilities (ensemble ideal-diagram)
  ;; first add arcs
  (flet ((print-an-assignment (type resource resource-model-name attack-case prob)
           (when *report-out-loud*
             (format t "~%Setting ~a prob of ~a ~a <- ~{~a ~a~^, ~} to ~5,4f" 
                     ...
                     prob))))
    (ask `[resource ,ensemble ? ?resource]
         #'(lambda (stuff)
             (declare (ignore stuff))
             (let ((resource-node (ideal:find-node (qualified-name ?resource) ideal-diagram))
                   (cond-case-alist nil)
...
(defclass search-control ()
  ((ensemble-name :initarg :ensemble-name :accessor ensemble-name)
   (ensemble :initarg :ensemble :accessor ensemble)
...
(defmethod initialize-resources-and-components ((sc search-control))
  (with-slots (resource-nodes component-nodes attack-nodes ensemble ensemble-name ideal-diagram remaining-states) sc
    (unless (and resource-nodes component-nodes)
      (ask `[resource ,ensemble ? ?resource]
	   #'(lambda (just)
	       (declare (ignore just))
	       (push (ideal:find-node (qualified-name ?resource) ideal-diagram) resource-nodes)))
      (ask `[attack ,ensemble ?name ?attack-type]
           #'(lambda (just)
               (declare (ignore just))
	...
	(let ((current-bit-position 1))
	  (ask `[and [component ,ensemble ?component-name ?component] 
		     ...
		     [possible-model ?component ?y]]
	       #'(lambda (just)
		   (declare (ignore just))
		   (let ((assertion (tell [selected-model ?component ?y] :justification :none)))
		     (setf (bit-pattern assertion) current-bit-position)
		     (setq current-bit-position (* current-bit-position 2))
		     (pushnew ?component all-components) 
		     (push (ideal:find-node (qualified-name ?component) ideal-diagram) component-nodes)
		     ))))
	(labels ((do-one (remaining-components partial-state)
		   (if (null remaining-components)
		       (push (cons (bit-vector-from-assertion-set partial-state) partial-state) remaining-states)
...
(defun set-up-search-control (ensemble-name ensemble-inside attack-models)
  (when attack-models 
    (loop for attack-model in attack-models 
	do (apply-attack-model attack-model ensemble-inside)))
  (let* ((ideal-model (build-ideal-model ensemble-inside))
	 (search-control (make-instance 'search-control
			   :ensemble ensemble-inside
			   :ensemble-name ensemble-name
			   :ideal-diagram ideal-model)))
    (initialize-resources-and-components search-control)
    search-control))

(defun create-initial-inputs (ensemble)
  (let ((input-descriptions (find-initial-inputs ensemble)))
...
(defrule expand-inside (:forward)
  ...
  then (block exit
	 (ask [corresponding-ensemble ?component ?e]
	      #'(lambda (x)
		  (declare (ignore x))
		  (return-from exit (values))))
	 ;; only build the inside once
	 (build-inside ?type ?component)))
...
(defmethod remove-all-selected-components ((sc search-control))
  (ask `[component ,(ensemble sc) ?component-name ?component]
       #'(lambda (just)
	   (declare (ignore just))
	   (ask [selected-model ?component ?]
		#'(lambda (just)
...
(defmethod capture-solution ((sc search-control))
  (with-slots (solution-states remaining-states) sc
    (let ((solution nil))
      (ask [selected-model ?component ?model]
	   #'(lambda (just)
	       (ask `[component ,(ensemble sc) ? ?component]
		    #'(lambda (just2)
			(declare (ignore just2))
           ...
           #'(lambda (just)
               ...
               (let ((found-it nil))
                 (ask [selected-model ?component ?model]
                      #'(lambda (just)
                          (setq found-it t)
                          (push (list ?component (ask-database-predication just)) selection-alist)))
                 (unless found-it
                   (push (list ?component) negations)
                   (ask `[not [selected-model ?component ?]]
                        #'(lambda (just)
                            (let ((entry (assoc ?component negations)))
...
(defmethod remove-all-component-evidence ((sc search-control))
  (let ((nodes nil))
    (with-slots (ensemble) sc
      (ask `[and [component ,ensemble ?component-name ?component] [possible-model ?component ?y]]
           #'(lambda (just)
               (declare (ignore just))
               (let* ((assertion (tell [selected-model ?component ?y] :justification :none))
...
(defmethod io-named ((e ensemble) name direction component-name)
  (let ((component (component-named e component-name)))
    (when component
...
(defmethod situation-named ((e ensemble) before-or-after component-name)
  (let ((component (component-named e component-name)))
    (when component
...
(defmethod interval-named ((e ensemble) before-or-after component-name)
  (let ((component (component-named e component-name)))
    (when component
...
(defmethod component-named ((e ensemble) name)
  (ask `[component ,e ,name ?component]
       #'(lambda (stuff)
	   (declare (ignore stuff))
	   (return-from component-named (values ?component)))))

(defmethod resource-named ((e ensemble) name)
  (ask `[resource ,e ,name ?resource]
       #'(lambda (stuff)
	   (declare (ignore stuff))
